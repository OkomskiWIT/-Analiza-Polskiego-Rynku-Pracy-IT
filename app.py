...
tab_pl, tab_global = st.tabs(["Rynek Polski & Zarobki", "Rynek Globalny"])

# --- Zakladka: Rynek Globalny ---
with tab_global:
    ...
    try:
        ...
        st.error(f"Blad ladowania danych globalnych: {e}")

# --- Zakladka: Rynek Polski ---
with tab_pl:
    ...
    try:
        ...
        if not df_pl.empty:
            ...
            for index, row in df_pl.iterrows():
                miasto = str(row['location'])
                # Rozbijamy po przecinku i bierzemy pierwsze miasto z brzegu, ignorujac "Brak (tylko zdalnie)"
                pierwsze_miasto = miasto.split(',')[0].strip()
                if pierwsze_miasto in coords:
                    map_data.append({
                        'lat': coords[pierwsze_miasto][0],
                        'lon': coords[pierwsze_miasto][1],
                    })
        ...
        st.error(f"Blad ladowania danych z Polski: {e}")
